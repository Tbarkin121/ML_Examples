...
class TDLambda():
    ...
    def train(self):
        ...
        with tf.GradientTape() as tape:
            q_s1_est = self.actor(state_t1_mod, training=True)
            q_s2_est = self.target(state_t2_mod, training=False)
            
            self.q_s1_gathered = tf.gather(q_s1_est, action_t1_mod, axis=1, batch_dims=1)
            self.q_s2_max = tf.expand_dims(tf.reduce_max(q_s2_est, axis=1), axis=1)
            
            self.returns = (reward_t2_mod + self.gamma*self.q_s2_max)*(1-done_mod)
