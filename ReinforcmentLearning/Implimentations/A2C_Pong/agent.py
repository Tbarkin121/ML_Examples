...
class ActorNet(tf.keras.Model):
  ...
  def __init__(self, num_actions: int, num_hidden_units: int):
    ...
    self.c1 = layers.Conv2D(filters=32, kernel_size=8, strides=(4,4), activation='relu')
    self.c2 = layers.Conv2D(filters=64, kernel_size=4, strides=(2,2), activation='relu')
    self.c3 = layers.Conv2D(filters=64, kernel_size=3, strides=(1,1), activation='relu')
...
class ACER():
    ...
    def reset_envs(self, env):
        self.state_t1 = tf.cast(env.reset(), tf.float32)

    # def env_step(action: np.ndarray, env) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    #   """Returns state, reward and done flag given an action."""
      
    #   env.step_async(action)
    #   state, reward, done, total_reward_dict = env.step_wait() 
    #   total_reward = [item['total_rewards'] for item in total_reward_dict]
    #   total_reward = tf.reshape(total_reward, shape=(num_env,1))
    # #   state, reward, done, total_reward_dict = env.step(action)
    #   return (state.astype(np.float32), np.array(reward, np.float32), np.array(done, np.int32), np.array(total_reward, np.float32))
      

    # def tf_env_step(action: tf.Tensor, env) -> List[tf.Tensor]:
    #   return tf.numpy_function(self.env_step, [action, env], [tf.float32, tf.float32, tf.int32, tf.float32])
    ...
    def take_n_steps(self, env, n_steps=1):
        ...
        for _ in range(n_steps):
            ...
            self.state_t2 = tf.cast(self.state_t2, tf.float32)
            self.action_t1 = tf.cast(tf.reshape(self.action_t1, shape=(self.num_env,1)),tf.int32)
            self.reward_t2 = tf.cast(tf.reshape(self.reward_t2, shape=(self.num_env,1)),tf.float32)
            self.done = tf.cast(tf.reshape(self.done, shape=(self.num_env,1)), dtype=tf.float32)
            # print(total_reward_dict)
            # self.total_reward = [item['total_rewards'] for item in total_reward_dict]
            # self.total_reward = tf.cast(tf.reshape(self.total_reward, shape=(self.num_env,1)),tf.float32)
